...
def train_step(x, y):
    ...
    # TopKCategoricalAccuracy で予測精度を記録
    train_prediction_accuracy.update_state(y, predictions)
...
def test_step(x, y):
    ...
    # TopKCategoricalAccuracy で予測精度を記録
    val_prediction_accuracy.update_state(y, predictions)
...
# 分類精度を記録するオブジェクトを追加
train_prediction_accuracy = tf.keras.metrics.TopKCategoricalAccuracy(k=1, name="train_prediction_accuracy")
val_prediction_accuracy = tf.keras.metrics.TopKCategoricalAccuracy(k=1, name="val_prediction_accuracy")
...
for epoch in range(epochs):
    ...
    for xtrain_set in tqdm(range(int(filenum / fileset))): #(filenum:314)/(fileset:2)=157
        ...
        for ll in range(fileset):
            # 入力データとラベルデータを読み込むB
            input_data1 = np.loadtxt("/home/user_05/program/input_data/R_4ele_pilot_true_50per_Python/R_50_"+str(ll+fileplus)+'.csv', delimiter=",")  # 入力データ (30000, 7)
            input_data2 = np.loadtxt("/home/user_05/program/input_data/theta_true_4ele_pilot_50per_Python/theta_true_50_"+str(ll+fileplus)+'.csv', delimiter=",")  # 入力データ (30000, 1)
            label_data = np.loadtxt("/home/user_05/program/label_data/d_4ele_pilot_true_50per_Python_01/d_50per_01_"+str(ll+fileplus)+'.csv',delimiter=",")  # 教師データ (30000, 995)
            
             # 横方向に連結 (N, 8)Traceback (most recent call last):
            #input_data1_norm = input_data1 / np.linalg.norm(input_data1, axis=1, keepdims=True)
            input_data2_2d = input_data2.reshape(-1, 1)
            #input_data2_sincos = np.stack([np.cos(input_data2), np.sin(input_data2)], axis=1) 
            #input_data = np.concatenate([input_data1, input_data2_sincos], axis=1) 
            input_data = np.concatenate([input_data1, input_data2_2d], axis=1)
    ...
    avg_train_pred_acc = train_prediction_accuracy.result()  # 訓練時の予測精度
    avg_val_pred_acc = val_prediction_accuracy.result()      # 検証時の予測精度
...
#モデルの保存
model.save('/home/user_05/program/model_save_50per_3000node_inputRandangle_pilot_true_Python') #モデルと重みの保存
#重みの保存
model.save_weights("/home/user_05/program/weight_datanum50per_3000node_inputRandangle_pilot_true_Python.h5", save_format="h5")    
# モデルの概要を出力
model.summary()
...
ax.set_xlabel('epoch')
#plt.show()
fig.savefig('/home/user_05/d_loss_TandA/d_loss_TandA_3000node_inputRandangle_pilot_true_Python.png')
...
#plt.show()
fig2.savefig('/home/user_05/d_accuracy_TandA/d_accuracy_TandA_inputRandangle_pilot_true_Python.png')
...
#plt.show()
fig3.savefig('/home/user_05/d_loss_T/d_loss_inputRandangle_pilot_true_Python.png')
...
#plt.show()
fig4.savefig('/home/user_05/d_accuracy_T/d_accuracy_inputRandangle_pilot_true_Python.png')
"""
# 訓練データの予測精度
fig5 = plt.figure()
...
#plt.show()
fig5.savefig('/home/user_05/d_prediction_accuracy_TandA/d_prediction_TandA_1000node_90_scaled_ver2.png')
"""
