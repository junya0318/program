...
for ll in tqdm(range(0,10)):
    ...
    for mm in range(thetasize_jammer1):
        ...
        for nn in range(thetasize_jammer2):
            ...
            dd = []
            SINR = []
            for d1 in np.arange(lam/2,500*lam,0.01):
    ...
    "-----------------------csv書き込み------------------------------"
    # 相関行列の書き込み
    output_dir = r'C:\Users\junya\OneDrive\ドキュメント\MATLAB\correlation_matrix_output'
    ...
    # 最適素子間隔の書き込み
    output_dir = r'C:\Users\junya\OneDrive\ドキュメント\MATLAB\optical_distance_output'
    if not os.path.exists(output_dir):
     os.makedirs(output_dir)
