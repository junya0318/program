...
for epoch in range(epochs):
    ...
    for xtrain_set in tqdm(range(int(filenum / fileset))): #(filenum:314)/(fileset:2)=157
        ...
        for ll in range(fileset):
            R_data = np.loadtxt(r''+str(ll+fileplus)+'.csv',delimiter=",")#D:/Program/R_d5per_random/R_d5per
            d_data = np.loadtxt(r''+str(ll+fileplus)+'.csv',delimiter=",")
            #shape...(98596, 7)
    ...
    if (epoch + 1) % 1 == 0:
        ...
        # 検証データの損失をEarlyStoppingオブジェクトに渡して早期終了を判定
            ...
            #break
#モデルの保存
model.save('model_save_datanum10per_6000node_DoAv2') #モデルと重みの保存
#重みの保存
model.save_weights("weight_datanum10per_6000node_DoAv2.h5", save_format="h5")    
# モデルの概要を出力
model.summary()
'''
9. 損失の推移をグラフにする
...
for test in range(int(testfilenum / testfileset)):
    ...
    for ll in tqdm(range(testfileset)):
        R_testdata = np.loadtxt(r'/home/user_05/program/R_d10per_random_60/R_10_' + str(ll + fileplus) + '.csv',delimiter=",")
        d_testdata = np.loadtxt(r'/home/user_05/program/d_10per_60_01/d_10per_60_01_' + str(ll + fileplus) + '.csv', delimiter=",")
    ...
    print("test_preds range:", np.min(test_preds), np.max(test_preds))
