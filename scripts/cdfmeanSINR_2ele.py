...
"------------------------到来角度---------------------"
new_model = tf.keras.models.load_model('/home/user_05/program/deeplearning_model/model_save_50per_3000node_MATLAB_0.1lam_sna10000_2ele_compare_ver2')
...
for theta in tqdm(range(100000)):#00)):#0000)):
    ...
    # 区間ごとにスライス（Python は 0-始まり・終端は「含まない」）
    sig1 = sig[:,      :10000]      # 1   ～10000   （1 万サンプル）
    sig2 = sig[:,  5000:15000]      # 5001～15000   （1 万サンプル）
    sig3 = sig[:, 10000:20000]      # 10001～20000  （1 万サンプル）
    ...
    # --- 信号+雑音行列 X ---
    X = sv @ sig + noise
    X1 = sv @ sig1 + noise[:,      :10000]
    X2 = sv @ sig2 + noise[:,  5000:15000]
    X3 = sv @ sig3 + noise[:, 10000:20000]

    # --- 相関行列Rの計算 ---
    R = (X @ X.conj().T) / num_samples
    
    # --- 区間ごとの相関行列 & flatten（R2_list 用）---
    R1 = (X1 @ X1.conj().T) / X1.shape[1]   # 分母=10000
    R2 = (X2 @ X2.conj().T) / X2.shape[1]
    R3 = (X3 @ X3.conj().T) / X3.shape[1]
    ...
    R2_input = np.array([
    first_row1[0].real,           # R11 (区間1)
    first_row1[1].real,           # Re R12 (区間1)
    first_row1[1].imag,           # Im R12 (区間1)
    first_row2[2].real,           # Re R13 (区間2)
    first_row2[2].imag,           # Im R13 (区間2)
    first_row3[3].real,           # Re R14 (区間3)
    first_row3[3].imag            # Im R14 (区間3)
], dtype=np.float32)              # ← ここで数値型指定
    ...
    #x = np.array(np.reshape(R2_input,(1,7)))
...
with open('/home/user_05/program/cdf_d50_100lam_3000node/cdf_d50_100lam_3000node_0.1lam_2ele_compare_ver2.csv', 'w', newline="") as f:
