...
model = MLP(3000, 3000, 3000, 3000, 995)#10, 100, 
...
for epoch in range(epochs):
    ...
    for xtrain_set in tqdm(range(int(filenum / fileset))): #(filenum:314)/(fileset:2)=157
        ...
        for ll in range(fileset):
            R_data = np.loadtxt(r'/home/user_05/program/R_d50per_random_6ele_0.5lam/R_50_'+str(ll+fileplus)+'.csv',delimiter=",")#D:/Program/R_d5per_random/R_d5per
            d_data = np.loadtxt(r'/home/user_05/program/d_50per_6ele_0.5lam_01/d_50per_01_'+str(ll+fileplus)+'.csv',delimiter=",")
            #shape...(98596, 7)
    ...
    if (epoch + 1) % 1 == 0:
        ...
        # 検証データの損失をEarlyStoppingオブジェクトに渡して早期終了を判定
            ...
            #break
#モデルの保存
model.save('model_save_50per_3000node_6ele_ver1') #モデルと重みの保存
#重みの保存
model.save_weights("weight_datanum50per_3000node_6ele_ver1.h5", save_format="h5")    
# モデルの概要を出力
model.summary()
'''
9. 損失の推移をグラフにする
...
ax.set_xlabel('epoch')
#plt.show()
fig.savefig('d_input6_loss_TandA_theta.png')
...
#plt.show()
fig2.savefig('d_input6_accuracy_TandA_theta.png')
...
#plt.show()
fig3.savefig('d_input6_loss_theta.png')
...
#plt.show()
fig4.savefig('d_input6_accuracy_theta')
...
for test in range(int(testfilenum / testfileset)):
    ...
    for ll in tqdm(range(testfileset)):
        R_testdata = np.loadtxt(r'/home/user_05/program/R_d5per_random_test/R_d5per_random_test/R_d5per_test' + str(ll + fileplus) + '.csv',delimiter=",")
        d_testdata = np.loadtxt(r'/home/user_05/program/d_5per_random_test/d_5per_random_test/d_5per_test_' + str(ll + fileplus) + '.csv', delimiter=",")
